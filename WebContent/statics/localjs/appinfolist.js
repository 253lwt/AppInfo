$("#queryCategoryLevel1").change(function(){
	...
	if(queryCategoryLevel1 != '' && queryCategoryLevel1 != null){
		$.ajax({
			type:"GET",//请求类型
			url:"/AppInfoSystem/appInfo/categorylevellist.json",//请求的url
...
$(".addVersion").on("click",function(){
	var obj = $(this);
	window.location.href="/AppInfoSystem/version/appversionadd?id="+obj.attr("appinfoid");
});
$(".modifyVersion").on("click",function(){
	...
	if(status == "1" || status == "3"){//待审核、审核未通过状态下才可以进行修改操作
		if(versionid == null || versionid == ""){
			...
			window.location.href="/AppInfoSystem/version/appversionmodify?vid="+ versionid + "&aid="+ appinfoid;
		}
	}else{
...
$(".modifyAppInfo").on("click",function(){
	...
	var status = obj.attr("status");
	if(status == "1" || status == "3"){//待审核、审核未通过状态下才可以进行修改操作
		window.location.href="/AppInfoSystem/appInfo/appinfomodify.json?id="+ obj.attr("appinfoid");
	}else{
...
var saleSwitchAjax = function(appId,obj){
	$.ajax({
		...
		error:function(data){
			if("open" === obj.attr("saleSwitch")){
				alert("恭喜您，【"+obj.attr("appsoftwarename")+"】的【上架】操作成功1");
			}else if("close" === obj.attr("saleSwitch")){
				alert("恭喜您，【"+obj.attr("appsoftwarename")+"】的【下架】操作成功2");
			}
		}
	});
};
...
$(".viewApp").on("click",function(){
	var obj = $(this);
	window.location.href="/AppInfoSystem/appInfo/flatform/app/appinfoview?id="+ obj.attr("appinfoid");
});
...
$(".deleteApp").on("click",function(){
	...
	if(confirm("你确定要删除APP应用【"+obj.attr("appsoftwarename")+"】及其所有的版本吗？")){
		$.ajax({
			type:"GET",
			url:"/AppInfoSystem/appInfo/flatform/app/delapp.json",
